...
router.post("/user/updateEmail", auth, async (req, res) => {
    try {
        ...
        if (!req.userId || !newEmail) return res.status(400).json({message:"Userid or the sent data is not valid."})
        ...
        if (existingUser.email === newEmail) {
            return res.status(400).json({message:"Email already exists. Enter a different email."})
        }

        const verificationCode = Math.floor(100000 + Math.random() * 900000).toString()

        await User.findByIdAndUpdate(req.userId, {tempEmail: newEmail, verificationCode, verified: false})
...
router.post("/user/verify-email",auth, async (req,res) => {
    try {
        ...
        if (user.tempEmail === "") return res.status(400).json({ message: "No email update requested." });

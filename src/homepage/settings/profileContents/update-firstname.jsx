...
const UpdateFirstName = ({ currFirstName, setUpdateFirstName }) => {
  const token = localStorage.getItem("token");
  const [newFirstName, setNewFirstName] = useState("" || currFirstName);
  ...
  const onSubmit = (event) => {
    ...
    if (!newFirstName.trim() || currFirstName === newFirstName.trim()) {
      return;
    }
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black/60 z-60">
      ...
      <div className="w-120 sm:w-150">
        <div className={DailyTabStyles}>
          ...
          <h1 className="text-3xl font-bold mt-8">First name</h1>
          <form className="flex flex-col gap-4" onSubmit={onSubmit}>
            <p className="-mb-3 text-xs ml-1">
              Enter your first name or preferred name.
            </p>
            <input
              ...
              value={newFirstName}
              onChange={(el) => setNewFirstName(el.target.value)}
            />

...
const UpdateLastName = ({ currLastName, setUpdateLastName }) => {
  const token = localStorage.getItem("token");
  const [newLastName, setNewLastName] = useState("" || currLastName);
  ...
  const onSubmit = (event) => {
    ...
    if (!newLastName.trim() || currLastName === newLastName.trim()) {
      return;
    }
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black/60 z-60">
      ...
      <div className="w-120 sm:w-150">
        <div className={DailyTabStyles}>
          ...
          <h1 className="text-3xl font-bold mt-8">Last name</h1>
          <form className="flex flex-col gap-4" onSubmit={onSubmit}>
            <p className="-mb-3 text-xs ml-1">Enter your last name.</p>
            <input
              ...
              value={newLastName}
              onChange={(el) => setNewLastName(el.target.value)}
            />
